...
diff   = eps;
% [U, ~] = eigs(L, k, diff);
% [U, ~] = eigs(L, k, 'SA');  % By Yazhou 20121210
% [U, ~] = eigs(L+eye(size(L)), k, diff); % By Yazhou 20130210
%-----------------% By Yazhou 20130210
[U,Val] = eig(L);
temp = diag(Val);
[~,idx]=sort(temp);
...
% U=[real(U),imag(U)];
C = kmeans(U, k, 'emptyAction', 'singleton');  % Yazhou 20121217
             
% now convert C to a n-by-k matrix containing the k indicator
% vectors as columns
% C = sparse(1:size(D, 1), C, 1);   % By Yazhou 20121210
